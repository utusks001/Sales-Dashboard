...
all_cities = df["Country"].unique()
city_selection = st.sidebar.checkbox("Select All Country", value=True)
if city_selection:
    selected_cities = all_cities
else:
    selected_cities = st.sidebar.multiselect("Select the Country:", options=all_cities, default=all_cities)

all_customer_types = df["Segment"].unique()
customer_type_selection = st.sidebar.checkbox("Select All Segment", value=True)
if customer_type_selection:
    selected_customer_types = all_customer_types
else:
    selected_customer_types = st.sidebar.multiselect("Select the Segment:", options=all_customer_types, default=all_customer_types)

all_genders = df["Segment"].unique()
gender_selection = st.sidebar.checkbox("Select All Segment", value=True)
if gender_selection:
    selected_genders = all_genders
else:
    selected_genders = st.sidebar.multiselect("Select the Segment:", options=all_genders, default=all_genders)
...
average_sale_by_transaction = int(round(df_selection["Sales"].mean()))
...
# SALES BY HOUR [LINE CHART]
sales_by_hour_city = df_selection.groupby(by=["Month", "Country"])[["Sales"]].sum().reset_index()
fig_hourly_sales = px.line(
    sales_by_hour_city,
    x="Month",
    y="Sales",
